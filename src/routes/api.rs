...
pub async fn accept_project_invite(
    ...
    let session_uid = req.ib_uid.or_else(|| {
        http_req.headers().get("ib-uid").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<i64>().ok())
    });

    let session_uid = match session_uid {
        Some(uid) => uid,
        None => return HttpResponse::BadRequest().json(PostResponse { success: false, message: "Missing ib-uid".to_string(), postid: None }),
    };

    let session_user = match req.ib_user.clone() {
        Some(u) => u.trim().to_string(),
        None => {
            match sqlx::query_scalar::<_, String>("SELECT username FROM user WHERE CONVERT(ib_uid USING utf8mb4) COLLATE utf8mb4_unicode_ci = ? LIMIT 1")
                .bind(session_uid.to_string())
                .fetch_optional(&state.db_pool)
                .await
            {
                Ok(Some(u)) => u,
                Ok(None) => return HttpResponse::BadRequest().json(PostResponse { success: false, message: "Missing ib-user (not found)".to_string(), postid: None }),
                Err(e) => {
                    eprintln!("Database error looking up username: {}", e);
                    return HttpResponse::BadRequest().json(PostResponse { success: false, message: format!("Missing ib-user: {}", e), postid: None });
                }
            }
        }
    };
...
pub async fn get_project_dm_messages(
    ...
    let sql = if req.before_id.is_some() {
