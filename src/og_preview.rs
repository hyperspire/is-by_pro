...
pub async fn get_og_preview(
    ...
    if url.contains("youtube.com/") || url.contains("youtu.be/") {
        ...
        if let Ok(oembed_url) = reqwest::Url::parse_with_params("https://rumble.com/api/Media/oembed.json", &[("url", url.as_str())]) {
            if let Ok(res) = client.get(oembed_url).send().await {
                if let Ok(json) = res.json::<serde_json::Value>().await {
                    let title = json.get("title").and_then(|v| v.as_str()).map(|s| s.to_string());
                    let author = json.get("author_name").and_then(|v| v.as_str()).map(|s| format!("Rumble Channel: {}", s));
                    let image = json.get("thumbnail_url").and_then(|v| v.as_str()).map(|s| s.to_string());
